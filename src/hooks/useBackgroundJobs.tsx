...
export const BackgroundJobsProvider: React.FC<BackgroundJobsProviderProps> = ({
  ...
  useEffect(() => {
    ...
    console.debug("[BackgroundJobs] Starting scheduled posts timer");
    scheduleNext();

    return () => {
      cancelled = true;
      if (timeoutId) clearTimeout(timeoutId);
      console.debug("[BackgroundJobs] Stopping scheduled posts timer");
    };
  }, [userId, organizationId]);
...
export const useGenerationJob = (context: string) => {
  const { getJobByContext, onJobComplete } =
    useBackgroundJobs();
