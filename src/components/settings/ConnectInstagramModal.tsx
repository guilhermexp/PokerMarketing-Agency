...
export function useInstagramAccounts(userId: string, organizationId?: string | null) {
  ...
  const fetchAccounts = useCallback(async () => {
    if (!userId || userId === "") return;  // Also check for empty string
  ...
  useEffect(() => {
    const fetchKey = `${userId}-${organizationId || 'personal'}`;
    if (userId && fetchedRef.current !== fetchKey) {
      fetchedRef.current = fetchKey;
      fetchAccounts();
    }
  }, [userId, organizationId, fetchAccounts]);
