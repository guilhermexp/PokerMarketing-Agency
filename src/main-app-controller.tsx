...
export function MainAppController({ routeView }: MainAppControllerProps) {
  ...
  const setScheduledPosts = useScheduledPostsStore((state) => state.setScheduledPosts);
  const publishingStates = useScheduledPostsStore((state) => state.publishingStates);
  ...
  const transformedGalleryImages: GalleryImage[] = useMemo(() => (swrGalleryImages || [])
    .filter((img) => {
  ...
  const transformedScheduledPosts: ScheduledPost[] = useMemo(() => (swrScheduledPosts || [])
    .map((post) => {
  ...
  const transformedCampaignsList: CampaignSummary[] = useMemo(() => (swrCampaigns || []).map(
    (c: DbCampaign) => ({
      id: c.id,
      name: c.name,
      status: c.status,
  ...
  useEffect(() => {
    setGalleryImages(transformedGalleryImages);
  }, [setGalleryImages, transformedGalleryImages]);

  useEffect(() => {
    setScheduledPosts(transformedScheduledPosts);
  }, [setScheduledPosts, transformedScheduledPosts]);

  useEffect(() => {
    setCampaignsList(transformedCampaignsList);
  }, [setCampaignsList, transformedCampaignsList]);

  // Sync SWR data to local state. Tournament events and schedules are also
  ...
  }, [setBrandProfile, initialData, brandProfile, userId, organizationId]);
  ...
  const handleCarouselUpdate = useCallback((updatedCarousel: CarouselScript) => {
    ...
    setCampaign({
      ...campaign,
      carousels: campaign.carousels?.map((carousel) =>
