...
import { runStudioAgentStream, getPendingInteraction } from '../lib/agent/claude/runner.mjs';
...
function buildAnswerMessage(answerPayload) {
  if (typeof answerPayload === 'string') return answerPayload;
  if (!answerPayload || typeof answerPayload !== 'object') {
    return 'Confirmado, pode continuar.';
  }

  const optionId = answerPayload.optionId ? String(answerPayload.optionId) : '';
  const text = answerPayload.text ? String(answerPayload.text) : '';

  if (optionId && text) {
    return `Resposta selecionada: ${optionId}. Complemento: ${text}`;
  }
  if (optionId) {
    return `Resposta selecionada: ${optionId}`;
  }
  if (text) {
    return text;
  }
...
export function registerAgentStudioRoutes(app) {
  ...
  app.post('/api/agent/studio/answer', async (req, res) => {
    ...
    const answerMessage = buildAnswerMessage(answer);

    const abortController = new AbortController();
    req.on('close', () => abortController.abort());

    setSseHeaders(res);
    res.write(`data: ${JSON.stringify({ type: 'thread', threadId: thread.id, resumed: true })}\n\n`);
