...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      const { user_id, name, description, logo_url, primary_color,
              secondary_color, tone_of_voice, settings } = req.body;
      ...
      const result = await sql`
        INSERT INTO brand_profiles (user_id, name, description, logo_url, primary_color,
                                    secondary_color, tone_of_voice, settings)
        VALUES (${user_id}, ${name}, ${description || null}, ${logo_url || null},
                ${primary_color}, ${secondary_color}, ${tone_of_voice},
    ...
    if (req.method === 'PUT') {
      const { id } = req.query;
      const { name, description, logo_url, primary_color, secondary_color,
              tone_of_voice, settings } = req.body;
