...
Quebrar componentes monolíticos (250K+ linhas) em módulos menores, testáveis e maintíveis, seguindo uma estratégia de migração gradual com feature flags e rollback seguro.
...
export const FEATURE_FLAGS = {
  // Ativar novas implementações gradualmente
  USE_NEW_API_CLIENT: process.env.VITE_USE_NEW_API_CLIENT === 'true',
  USE_NEW_CLIPS_TAB: process.env.VITE_USE_NEW_CLIPS_TAB === 'true',
  USE_NEW_FLYER_GENERATOR: process.env.VITE_USE_NEW_FLYER_GENERATOR === 'true',
  USE_NEW_CAROUSEL_TAB: process.env.VITE_USE_NEW_CAROUSEL_TAB === 'true',
  USE_NEW_IMAGE_PREVIEW: process.env.VITE_USE_NEW_IMAGE_PREVIEW === 'true',
  USE_ZUSTAND_STORES: process.env.VITE_USE_ZUSTAND_STORES === 'true',
} as const;
...
import { ClipsTabLegacy } from './ClipsTabLegacy';
import { ClipsTab as ClipsTabNew } from '@/components/clips/ClipsTab';

export const ClipsTab = isFeatureEnabled('USE_NEW_CLIPS_TAB')
  ? ClipsTabNew
  : ClipsTabLegacy;
```

### Migração Paralela - Passo a Passo:

1. **Manter código antigo intacto** (renomear para `*Legacy`)
2. **Criar nova implementação** na estrutura proposta
3. **Implementar feature flag** para alternar
4. **Testar em staging** com flag habilitada
5. **Habilitar em produção** para % dos usuários
6. **Monitorar métricas** por 1 semana
7. **Remover código legacy** após validação
...
```bash
# 1. Desabilitar feature flag (imediato)
# .env.production
...
- [ ] Manter feature flag para toggle
...
| Componentes principais | < 300 linhas |
| Hooks | < 200 linhas cada |
| Serviços | < 400 linhas cada |
| Cobertura de testes | > 80% |
| Arquivos por feature | 8-15 arquivos |
...
| 2 | apiClient.ts | API client modularizado |
...
- [ ] Feature flags removidas (código legacy deletado)
- [ ] Documentação atualizada
